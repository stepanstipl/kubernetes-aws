...
GRAFANA_PASSWORD=${GRAFANA_PASSWORD_FILE:-"${SECRETS_PATH}/grafana_password"}
[[ -f "$GRAFANA_PASSWORD" ]] && GRAFANA_PASSWORD="$(cat $GRAFANA_PASSWORD)" || GRAFANA_PASSWORD=${GRAFANA_PASSWORD:?"\$GRAFANA_PASSWORD not set"}
GRAFANA_PORT=${GRAFANA_PORT:-3000}

INFLUXDB_HOST=${INFLUXDB_HOST:-"monitoring-influxdb"}
INFLUXDB_DATABASE=${INFLUXDB_DATABASE:-k8s}
INFLUXDB_PASSWORD=${INFLUXDB_PASSWORD_FILE:-"${SECRETS_PATH}/influxdb_password"}
[[ -f "$INFLUXDB_PASSWORD" ]] && INFLUXDB_PASSWORD="$(cat $INFLUXDB_PASSWORD)" || INFLUXDB_PASSWORD=${INFLUXDB_PASSWORD:?"\$INFLUXDB_PASSWORD not set"}
